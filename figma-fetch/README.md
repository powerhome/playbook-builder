...
Fetch Figma designs via the REST API and produce compact Playbook page specs --
...
```bash
cd figma-fetch && npm install && npm run build
```
...
```bash
cd figma-fetch && npm run test:package
```

### Test against Figma (integration)

With `FIGMA_TOKEN` set, run `figma-fetch` with a real design URL and confirm JSON under `figma-fetch/output/` (or `--stdout`).

## Usage

```bash
# Produce a compact page spec → auto-saved to figma-fetch/output/
figma-fetch/bin/figma-fetch \
  --url "https://www.figma.com/design/abc123/MyFile?node-id=358-93336"

# Custom output filename (must be within figma-fetch/output/)
figma-fetch/bin/figma-fetch \
  --url "https://..." --output figma-fetch/output/my-spec.json
...
figma-fetch/bin/figma-fetch --url "https://..." --stdout

# Limit traversal depth
figma-fetch/bin/figma-fetch --url "https://..." --depth 5
```
...
| Aspect              | MCP workflow           | figma-fetch               |
|---------------------|------------------------|---------------------------|
